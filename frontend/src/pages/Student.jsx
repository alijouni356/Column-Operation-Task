...
import PartialQuotient from "../pages/PartialQuotient";
...
export default function Student() {
  ...
  const handleDivisionSubmit = async (isCorrect, quotient) => {
    ...
    setResult({ ...res.data, pqDone: true });
    if (!isCorrect) {
      setShake(true);
      setTimeout(() => setShake(false), 600);
    }
  ...
  const opSymbol = { "+": "＋", "-": "－", "*": "×", "/": "÷" };
  const isDivision = problem?.operation === "/";
  ...
  return (
    <div className="page student-page">
      ...
      <div className="student-center">
        ...
        {!loading && problem && (
          <div className={`card solve-card ${isDivision ? "solve-card-wide" : ""} ${shake ? "shake" : ""}`}>
              ...
              <>
                ...
                {result?.pqDone && (
                  <div className="pq-next">
                    ...
                    <button className="submit-btn mt" onClick={loadProblem}>
                      Next Problem →
                    </button>
                  </div>
                )}
